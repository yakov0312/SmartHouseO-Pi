 ...
 */
CommandResult WakeOnLanManager::execute(const CommandRequest& cmd, bool streamAllowed)
{
	...
	if (it->second == nullptr)
	{
		...
		return {"Command is not enabled"};
	}

	// No command here uses stream the param here is just for the overload
...
CommandResult WakeOnLanManager::handleDevices(const CommandRequest& cmd)
{
	...
	if (cmd.args.empty())
		return {"Invalid command call. usage WakeOnLan Devices Get/Add/Remove"};
	...
	if (cmd.args[0] == "Add")
	{
		...
		if (cmd.args.size() < ADD_ARGS) // Add, Device, Mac
			return {"Invalid command call. usage WakeOnLan Devices Add <device> <mac>"};

		m_config.add(DEVICES_SECTION, cmd.args[1], cmd.args[2]);

		return {"Added device " + cmd.args[1]};
	}
	...
	if (cmd.args[0] == "Remove")
	{
		...
		if (cmd.args.size() < REMOVE_ARGS) // Remove, Device
			return {"Invalid command call. usage WakeOnLan Devices Remove <device>"};

		m_config.remove(DEVICES_SECTION, cmd.args[1]);

		return {"Removed device " + cmd.args[1]};
	}

	return {"Arg is invalid or unsupported now"};
}
...
{
	if (cmd.args.empty())
		return {"Invalid command call. usage WakeOnLan Wake <DeviceName>"};
	...
	if (mac.empty())
		return {"Device " + cmd.args[0] + " Does not exist"};
	...
	return {"Sent WoL packet to " + cmd.args[0]};
}
