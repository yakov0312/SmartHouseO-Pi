	...
	while (true)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			else
			{
				...
				if (it != m_clients.end())
				{
					std::shared_ptr<Connection> client = it->second;
					if (events[i].events & EPOLLIN)
						handleClient(client);

					if (events[i].events & EPOLLOUT)
						handleResponses(client);

					if (!client->keepAlive.load())
...
void Server::handleClient(const std::shared_ptr<Connection>& client)
{
	ConnectionIO& io = client->io;

	const uint16_t readSize = m_protocolManager->getAvailableInputSpace(client->connectionId);
	if (readSize != 0)
	{
	...
	m_protocolManager->process(client->connectionId);
}

void Server::handleResponses(const std::shared_ptr<Connection>& client) const
{
	ConnectionIO& io = client->io;
	...
	if (bytes == -1)
	{
		Logger::get().error("Failed sending response to client id=" + std::to_string(client->connectionId));
		return;
	}
	...
	else
	{
		...
		event.events = EPOLLIN;
		event.data.u64 = client->connectionId;

		epoll_ctl(m_epollFd, EPOLL_CTL_MOD, client->io.fd.load(), &event);
	}
}

void Server::closeConnection(const std::shared_ptr<Connection>& client)
{
	close(client->io.fd.load());
	client->io.fd.store(-1);

	m_protocolManager->removeUser(client->connectionId);
	m_clients.erase(client->connectionId);
}
